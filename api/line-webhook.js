...
遇到以上範圍以外的問題（包含商品推薦、尺寸建議、促銷活動、庫存查詢、點數、會員等），一律回覆：
「這個問題需要請真人客服確認，請輸入「真人」，我們會在工作時間為您服務 😊」
禁止自行推斷、禁止用訓練資料補充任何答案。
...
【取消訂單】
- 下單後 12 小時內可自助取消：https://enamor-line-bot.vercel.app/cancel.html
...
【情緒偵測】
- 偵測到明顯負面情緒時，回覆開頭說「感覺您有點不開心 🙏」，語氣誠懇，末尾加 ###EMOTION###`;
...
const QUICK_REPLIES = {
  '1': '折扣後滿 NT$999 宅配和超商取貨都免運喔！免運是以折扣後金額為準，外島不在範圍內。有其他問題歡迎繼續問我 😊',
  '2': '客服服務時間是週一至週五 09:00–12:00 / 13:00–17:00。非服務時間留言也沒關係，下個工作日會盡快回覆 💌',
...
module.exports = async function handler(req, res) {
  ...
  for (const event of events) {
    ...
    if (CANCEL_TRIGGERS.some(w => userText.includes(w))) {
      await replyToLine(replyToken, '下單後 12 小時內可自助取消訂單：\nhttps://enamor-line-bot.vercel.app/cancel.html\n\n超過 12 小時請輸入「真人」由客服協助。');
      continue;
    }
    ...
    try {
      const systemPrompt = await getSystemPrompt();
      const msgsToSend = messages.slice(-12);
